...
5. **Github: my PRs.** `gh search prs --author={github} --created="YYYY-MM-DD..YYYY-MM-DD" --json number,title,repository,state,url,closedAt,createdAt`. The `state` field returns `"merged"`, `"closed"`, or `"open"` — exclude PRs where state is `"closed"` (closed without merging).
