...
> Vendored from [pproenca/dot-skills](https://github.com/pproenca/dot-skills) at `cf93c57cac89d6fc3e4194686000411567f5caf3` (v0.2.1, MIT — see [LICENSE](LICENSE)). Local adaptations: Related Skills point at deftly's artifacts, a layer boundary was added, and upstream authoring tooling references were removed.
...
This is the **thinking layer** that sits above artifact-level simplification: the `code-simplifier` and `structural-simplification` reviewer lanes in `solo-agents-team-review` apply a methodology to a known shape of artifact. This skill is the methodology itself — how to arrive at the simple shape in the first place.
...
This skill is **not** for cleaning up code that already does the right thing — that is the `code-simplifier` reviewer lane. Use this when the *approach itself* is what needs to get simpler.
...
- `solo-agents-team-review` [`templates/code-simplifier.md`](../solo-agents-team-review/templates/code-simplifier.md) — local, behavior-preserving refactoring once the right approach is known (this skill ends, that lane begins)
- `solo-agents-team-review` [`templates/structural-simplification.md`](../solo-agents-team-review/templates/structural-simplification.md) — structural deletions and code-judo at review time
- [`solo-agents-team-code`](../solo-agents-team-code/SKILL.md) — hosts the plan-simplifier judgment lane that applies this skill to a proposed decomposition before dispatch

## Authoring Note

These moves are **load-bearing**, not decorative. The wrong default each rule corrects is named in the rule itself — if a rule restates something a capable model already does correctly, cut it. Prove coverage on real complex-problem prompts, not by hitting a rule count.
