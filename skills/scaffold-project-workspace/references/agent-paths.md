...
| Codex CLI   | `AGENTS.md` (hierarchical: read from CWD up to repo root, plus `~/AGENTS.md` and `/AGENTS.md`) | Layered scan of `.agents/skills/` from CWD up to repo root, then `~/.agents/skills/`, `/etc/codex/skills/`, and built-ins | `~/.codex/config.toml` (user). Project-level `.codex/config.toml` is documented at developers.openai.com/codex/config-basic (loaded after `codex trust`), but does not appear in the Rust config-loader README's layer list — **conflicting sources; verify against installed version** before writing project config. `--sandbox` flag + `[sandbox_workspace_write]` policy | None documented as standard. `.codex/config.toml` is project-shared by default; no `.codex/config.local.toml` convention surfaced in docs |
